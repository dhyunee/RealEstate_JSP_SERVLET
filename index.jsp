...
<%
	...
	List<MainDto> mainlist = (ArrayList<MainDto>) request.getAttribute("mainList");
	List<SidoCodeDto> sidolist = (ArrayList<SidoCodeDto>) request.getAttribute("sidoList");
	List<GugunCodeDto> gugunlist = (ArrayList<GugunCodeDto>) request.getAttribute("gugunList");
	List<DongCodeDto> donglist = (ArrayList<DongCodeDto>) request.getAttribute("dongList");
%>
<!DOCTYPE html>
<html lang="en">
...
<body>
	...
	<section class="section main-banner" id="top" data-section="section1">
								...
								<div class="btn-group" style="margin-top: 30px;">
									<select id="wideCity"
										...
										<form method="post" action=main>
											<input type="hidden" name="action" value="sidolist">
											<option id="sidooption">도/광역시</option>

											<%
												for (SidoCodeDto sidoname : sidolist) {
											%>
											<option name="sidoselect" value=<%=sidoname.getCode()%>><%=sidoname.getName()%></option>
											<%
												}
											%>
										</form>
									</select> <select id="wideCity"
										class="form-select bg-secondary btn-lg text-light"
										style="margin: 10px;">

										<!-- <ul class="dropdown-menu"> -->
										<form method="post" action="main">
											<input type="hidden" name="action" value="gugunlist">
											<option id="gugunoption">구/군</option>

											<%
												for (GugunCodeDto gugunname : gugunlist) {
											%>


											<option name="gugunselect" value=<%=gugunname.getCode()%>><%=gugunname.getName()%></option>
											<%
												}
											%>
										</form>
										...
										<form method="post" action="main">
											<input type="hidden" name="action" value="donglist">
											<!-- <ul class="dropdown-menu"> -->
											<option id="dongoption">읍/면/동</option>

											<%
												for (DongCodeDto dongname : donglist) {
											%>

											<option name="dongselect" value=<%=dongname.getName()%>><%=dongname.getName()%></option>
											<%
												}
											%>
										</form>
		...
		document.querySelector("#logout").click=function(){
		    let response = await fetch("<%=contextPath%>/logout", "POST");
		}
        function fnMove(){
					...
					<div class="col" style="align-items: center;">
						<div class="card"
							...
							<form action="main" method="post">
								<input type="hidden" name="actionc" value="mainlist">
								<%
									for (MainDto mlist : mainlist) {
								%>
								<div class="card mb-3"
									...
									<div class="row g-0" style="padding-left: 10px;">
										...
										<div class="col-md-8">
											<div class="card-body">
												...
												<h6 class="card-title">
													<strong><%=mlist.getAptname()%></strong>
												</h6>
												<p class="card-text">
													<b><%=mlist.getDong()%></b>
												</p>
												<p class="card-text">
													<small class="text-muted">거래금액: <%=mlist.getdealAmount()%></small>
												</p>
												<p class="card-text">
													<small class="text-muted">건설연도: <%=mlist.getBuildyear()%></small>
												</p>
											</div>
										</div>
									</div>
